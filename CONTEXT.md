...
- **Ribbon view**: panel, popup or draft settings preview, sharing the shader and audio module. Per-instance curvature and fullness scale rendering only, leaving shared motion and analysis unchanged. Audio-reactive colors stay within the selected palette and can be disabled for the original fixed gradient.
