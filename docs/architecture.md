...
`RibbonMotion` accepts analyzer features and elapsed seconds, returning a `RibbonShape`. It has no Qt or PipeWire dependency. The worker owns one instance and publishes arch, counter-bend, horizontal bias, filament opening, lift and lean with the same features and phase. Relative band envelopes choose the broad shape; energy and mid changes against slower memories add phrase response. Six critically damped states retain position and velocity when their targets change. No timer chooses shapes and no random motion is added.

Lift and lean respond more slowly than the original four coordinates. Lift follows changes in `log1p(12 * rms) / log(13)` against a 2.5-second exponential reference. Lean follows the pre-normalization spectral balance against its own reference. These inputs prevent display gain adaptation from inventing a crescendo on steady audio. The targets are bounded to ±1 and attenuated by audible energy. Their critically damped frequencies are 2.4 and 2.0 radians per second. References initialize from current input after silence; published positions stay continuous. See the [slow base motion report](slow-base-motion.md).

The worker advances this state every iteration, independently of GUI sampling, frame limit, visibility or per-applet sensitivity. It keeps its 10 ms cadence while a stream can deliver audio or the envelope is fading; after an error with no negotiated format and energy below 0.0005, it waits until the next 100 ms routing poll. Each step is limited to 100 ms after a stall. Below energy 0.002 it holds the current shape and clears velocity, while the normal energy envelope fades the ribbon. New views read the current snapshot immediately. Initialization failure destroys the motion state with the rest of the pipeline. See [shared motion verification](shared-motion.md).
...
The broad path combines an anchored parabolic arch and a counter-bend along a horizontally biased coordinate. Slow lift and lean displace the veil and all filaments together, keeping the endpoints anchored and respecting the curvature setting. A smooth headroom limit reduces their contribution when the original curve already occupies much of the height. Reduced motion fixes both coordinates at zero. A procedural displacement provides continuity, bounded to 0.72 logical pixels at 40 px height. Filament opening follows the shared shape instead of a separate periodic fold. Shader and Canvas use the same broad centerline and independent mid-accent deformation. Canvas intentionally omits the travelling ripple and individual filaments. A previously loaded native plugin without shape fields gets a gentle fixed form until the next normal reload; missing lift and lean fields default to zero.
